...
class Player {
  constructor(x, y) {
    ...
    this.height = 30;
    this.moveSpeed = 1;
    this.moveForce = 0.01; // Fuerza horizontal de movimiento
    this.jumpForce = 0.5; // Fuerza de salto 
    this.rotationSpeed = 0.05; // Velocidad angular
    ...
    this.feet = Bodies.rectangle(x, -5 + y + this.height / 2, 5, 5, 
      {
        isSensor: true,
        density: 0.8,
      }
    ...
    Matter.Body.setCentre(this.composite, {x: 0, y: -this.height / 4  }, true); // no useful  
    // Matter.Body.setInertia(this.composite, Infinity);// no useful  
  ...
  /**
   * isGrounded: devuelve true si el jugador está en el suelo
   * comprueba si el angulo del jugador esta entre 45 y -45 grados, pero en radianes
   */
  isGrounded() {
    // modulo de 360° = 2PI
  ...
  draw() {
    ...
    if (this.isOnAir()) fill(155, 155, 0);
    else fill(0, 0, 155);
    // dibujar rectangulo a partir this.body vertices
    beginShape();
